...
const KiviCareTechStack = () => {
    return (
        <section className="w-full py-12 md:py-16 lg:py-24 bg-gradient-to-r from-sky-900 via-sky-800 to-teal-900 overflow-hidden relative">
            <div className="max-w-7xl mx-auto px-6 text-center text-white relative z-10">

                <div className="inline-flex items-center gap-2 px-6 py-2 mb-8 rounded-full border border-sky-400/40 text-[10px] font-bold uppercase tracking-[0.25em] bg-white/5 backdrop-blur-sm">
                    <span className="w-2 h-2 rounded-full bg-teal-400 animate-pulse"></span>
                    Clinical Tech-Stack
                </div>

                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-semibold mb-6 leading-tight tracking-tight  px-4">
                    Trusted Infrastructure for <span className="text-sky-300 font-bold">Healthcare</span> Precision
                </h2>

                <p className="max-w-2xl sm:max-w-4xl mx-auto text-sky-100/70 mb-12 lg:mb-16 text-sm sm:text-lg tracking-tight font-medium leading-relaxed px-4">
                    We use secure, HIPAA-compliant, and high-performance technologies to build robust
                    hospital management solutions. KiviCare is engineered for zero-failure clinical
                    operations and maximum data security.
                </p>
                ...
                <div className="flex flex-wrap justify-center gap-6">
                    {techStack.map((item, index) => (
                        <div
                            key={index}
                            className="group relative flex items-center gap-3 sm:gap-4 px-6 py-3 sm:px-10 sm:py-5 bg-white/5 backdrop-blur-xl rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 shadow-2xl hover:shadow-sky-500/20 transition-all duration-500 hover:-translate-y-2 cursor-pointer overflow-hidden"
                        >
            ...
            <div className="absolute top-0 left-0 w-96 h-96 bg-sky-500/10 rounded-full blur-[120px] -z-0" />
            <div className="absolute bottom-0 right-0 w-96 h-96 bg-teal-500/10 rounded-full blur-[120px] -z-0" />
        </section>
    );
};

export default KiviCareTechStack;
