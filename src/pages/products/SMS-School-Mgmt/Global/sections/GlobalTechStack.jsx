...
const GlobalTechStack = () => {
    return (
        <section className="w-full py-12 md:py-16 lg:py-24 bg-gradient-to-r from-slate-900 to-red-900 relative">
            <div className="max-w-7xl mx-auto px-6 text-center text-white">

                <div className="inline-flex items-center gap-2 px-5 py-2 mb-8 rounded-full border border-yellow-500/40 text-xs font-bold uppercase tracking-widest bg-white/5">
                    <span className="w-2 h-2 rounded-full bg-yellow-500"></span>
                    Governance Tech-Stack
                </div>

                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-semibold mb-6 leading-tight">
                    Reliable Infrastructure for Global Scale
                </h2>
                ...
                <p className="max-w-2xl sm:max-w-4xl mx-auto text-slate-300 mb-12 lg:mb-16 text-sm sm:text-lg tracking-tight font-medium px-4">
                    ...
                    university ERP systems. Global School Management is engineered for peak loads
                ...
                <div className="flex flex-wrap justify-center gap-6">
                    {techStack.map((item, index) => (
                        <div
                            key={index}
                            className="group relative flex items-center gap-3 px-6 py-3 sm:px-8 sm:py-4 bg-slate-900/60 backdrop-blur-md rounded-xl sm:rounded-2xl border border-white/5 shadow-2xl hover:shadow-yellow-500/10 transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                        >
                            <img
                                src={item.img}
                                alt={item.name}
...
export default GlobalTechStack;
