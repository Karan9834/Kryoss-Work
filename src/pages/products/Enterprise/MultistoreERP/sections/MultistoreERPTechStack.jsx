...
const MultistoreERPTechStack = () => {
    return (
        <section className="w-full py-12 md:py-16 lg:py-24 bg-gradient-to-r from-indigo-900 via-indigo-800 to-teal-900 overflow-hidden relative">
            <div className="max-w-7xl mx-auto px-6 text-center text-white relative z-10">

                <div className="inline-flex items-center gap-2 px-6 py-2 mb-8 rounded-full border border-indigo-400/40 text-[10px] font-bold uppercase tracking-[0.25em] bg-white/5 backdrop-blur-sm">
                    <span className="w-2 h-2 rounded-full bg-teal-400 animate-pulse"></span>
                    Modern Tech-Stack
                </div>

                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-semibold mb-6 leading-tight tracking-tight  px-4">
                    Built for <span className="text-teal-400 font-bold">Unstoppable</span> & <span className="text-indigo-400 font-bold">Global Scale</span>
                </h2>

                <p className="max-w-2xl sm:max-w-4xl mx-auto text-teal-100/70 mb-12 lg:mb-16 text-sm sm:text-lg tracking-tight font-medium leading-relaxed px-4">
                    Multistore ERP is powered by industrial-grade technologies to guarantee data integrity, 
                    zero-latency branch synchronization, and 100% uptime for your retail empire.
                </p>
                ...
                <div className="flex flex-wrap justify-center gap-6">
                    {techStack.map((item, index) => (
                        <div
                            key={index}
                            className="group relative flex items-center gap-3 sm:gap-4 px-6 py-3 sm:px-8 sm:py-4 bg-white/5 backdrop-blur-xl rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 shadow-2xl hover:shadow-teal-500/20 transition-all duration-500 hover:-translate-y-2 cursor-pointer overflow-hidden"
                        >
            ...
            <div className="absolute top-0 left-0 w-96 h-96 bg-indigo-500/10 rounded-full blur-[120px] -z-0" />
            <div className="absolute bottom-0 right-0 w-96 h-96 bg-teal-500/10 rounded-full blur-[120px] -z-0" />
        </section>
    );
};

export default MultistoreERPTechStack;
