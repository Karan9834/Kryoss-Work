      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {reasons.map((item, i) => (
            <div
              key={i}
              className={`rounded-2xl bg-white border ${item.borderColor} p-6 shadow-sm transition-all duration-300 ${item.hoverBg} ${item.hoverBorder} hover:shadow-md`}
            >
              {/* ICON */}
              <div className={`w-12 h-12 mb-5 rounded-xl flex items-center justify-center ${item.bgColor} ${item.iconColor} transition-colors duration-300`}>
                {item.icon}
              </div>
