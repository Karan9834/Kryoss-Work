...
import data from './EntertainmentOTTData.json';
import Testimonials from '../../../components/home/Testimonials';
import Contact from '../../../components/home/Contact';

const EntertainmentOTTApps = () => {
    ...
    return (
        <div className="w-full bg-white relative font-sans overflow-hidden">
            ...
            <section
                ...
                style={{
                    backgroundImage: "linear-gradient(to right, rgba(0,0,0,0.7), rgba(0,0,0,0.4)), url('https://kryosssoftech.org/icons/banner/entertainment.png')",
            ...
            >
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="relative z-10 container mx-auto px-4">
                    <h1 className="text-4xl md:text-6xl font-extrabold mb-6 tracking-tight">Entertainment & <span className="text-orange-500">OTT</span></h1>
                    <p className="text-lg md:text-2xl text-gray-200 max-w-3xl mx-auto font-light leading-relaxed">
                        Immersive entertainment and OTT platform solutions crafted for the digital streaming era.
                    </p>
                </motion.div>
            </section>
            ...
            <section className="bg-gray-100/80 py-4 border-b border-gray-200 backdrop-blur-md sticky top-0 z-40 shadow-sm">
                <div className="container mx-auto px-4 flex flex-wrap items-center text-sm md:text-base font-semibold text-gray-600">
                    ...
                    <span className="text-orange-500">Entertainment & OTT</span>
                </div>
            </section>
                ...
                <div className="container mx-auto px-4 lg:px-8">
                    <div className="text-center mb-12">
                        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-3xl md:text-4xl font-bold text-black mb-4">
                            Our <span className="text-orange-500">Entertainment & OTT</span> Portfolio
                        </motion.h2>
                        <div className="w-24 h-1 bg-orange-500 mx-auto rounded-full"></div>
                    </div>
                    <div className="flex flex-wrap justify-center gap-6 lg:gap-10 mb-8 px-2">
                        {tabs.map((tab) => (
                            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`pb-2 text-sm lg:text-[15px] font-bold uppercase transition-colors duration-300 border-b-2 tracking-wide ${activeTab === tab.id ? 'border-orange-500 text-black' : 'border-transparent text-gray-500 hover:text-orange-500 hover:border-orange-500/50'}`}>{tab.name}</button>
                        ))}
                    </div>
                </div>
                <div className="w-full mt-8 min-h-[500px]">
                    <AnimatePresence mode="wait">
                        <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }} className="flex flex-col w-full">
                            ...
                            {(!sectionsData[activeTab] || sectionsData[activeTab].length === 0) && (
                                <div className="w-full py-20 flex flex-col items-center justify-center text-center">
                                    ...
                                    <p className="text-gray-500 max-w-md">We are currently curating our best Entertainment & OTT works. Check back later!</p>
                                </div>
                            )}
                        </motion.div>
                    </AnimatePresence>
...
export default EntertainmentOTTApps;
